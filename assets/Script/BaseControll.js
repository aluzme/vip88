...
module.exports = {
	...
	util:        {},
	sslPem: 	 {},
	connect: function(url, path = '/', port = false, ss = true) {
		if (!this.isConnected) {
			const urlGame           = "ws" + (ss ? "s" : "") + "://" + url + (!!port ? ":" + port : "") + path;
			if (cc.sys.isBrowser) {
	...
	send: function (message) {
        ...
        const interval = setInterval(() => {
          ...
          } else if (thing._socket.readyState === 1) {
            thing._socket.send(this._encodeMessage(message));
            clearInterval(interval)
          }
          currentAttempt++
        }, intervalTime)
